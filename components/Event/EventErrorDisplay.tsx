// components/Event/EventErrorDisplay.tsx

'use client';

import { useState, useEffect } from 'react';
import { X, AlertCircle } from 'lucide-react';
import { RSVP_ERROR_ACTION_DESCRIPTIONS } from '@/lib/constants/errorKeys';
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";

export function EventErrorDisplay() {

  const [error, setError] = useState<{ message: string; action: string } | null>(null);

  useEffect(() => {
    const checkErrors = () => {
      // Check all RSVP error types
      for (const [key, action] of Object.entries(RSVP_ERROR_ACTION_DESCRIPTIONS)) {
